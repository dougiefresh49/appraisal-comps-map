...
export async function buildChatPrompt(
  ...
  const systemParts: string[] = [
    ...
    "",
    "## Past Reports / Historical Comparables",
    "The database contains **past appraisal reports** stored as projects with `is_reference=true`. These are previous appraisal engagements that contain historical comparables.",
    "- When the user asks about 'past reports', 'old reports', 'historical comps', or 'comps used in previous reports', they mean these reference projects.",
    "- Each 'report' is a row in the projects table. A report's comparables are in the comparables + comp_parsed_data tables, linked by project_id.",
    "- To find past reports: call search_all_projects with `include_reference='true'` or `include_reference='only'`. You can also filter by `property_type_search` (e.g. 'office', 'warehouse').",
    "- After finding reference project IDs, use list_project_comps and query_comp_data with those project_ids to retrieve historical comp data.",
    "- Do NOT call search_all_projects repeatedly with the same arguments. If a search returns no results, report that to the user rather than retrying.",
    "",
    "## Available Tools",
