...
| | |
|---|---|
| **Triggered by** | `POST /api/photos` → n8n `/subject-photos-save-input` |
| **What n8n does** | Receives photo list JSON → Writes `input.json` file to Google Drive |
| **Replacement difficulty** | Easy — Single Drive file write, could use Drive API with service account |
...
| | |
|---|---|
| **Triggered by** | `POST /api/comps-folder-list` → n8n `/comps-folder-list` |
| **What n8n does** | Lists subfolders in a Drive folder for comp discovery |
| **Replacement difficulty** | Easy — Simple Drive API folder listing |

### 5. Comp Folder Details

| | |
|---|---|
| **Triggered by** | `POST /api/comps-folder-details` → n8n `/comps-folder-details` |
| **What n8n does** | Gets metadata and file list for a specific comp folder |
| **Replacement difficulty** | Easy — Drive API file listing |
...
| | |
|---|---|
| **Triggered by** | Cover page client → n8n `/subject-photo-data` |
| **What n8n does** | Fetches cover photo metadata from Drive |
| **Replacement difficulty** | Easy — Single Drive file metadata fetch |
...
### Phase 1 — Easy wins (minimal n8n knowledge needed)
1. **Cover photo data** — Simple metadata fetch
2. **Comp folder list/details** — Drive API folder listing
3. **Photo export (input.json)** — Single file write

### Phase 2 — Medium (reuse existing patterns)
4. **Photo analysis** — Drive folder download + Gemini (replicate document processing pattern)
5. **Comp parser** — Drive download + Gemini extraction

### Phase 3 — Hard (requires Spreadsheet integration)
6. **Comp data refresh** — Need Sheets API, complex spreadsheet structure
7. **Comp exists check** — Need Sheets API
8. **Project creation** — Need Drive + Sheets API, understand project conventions
