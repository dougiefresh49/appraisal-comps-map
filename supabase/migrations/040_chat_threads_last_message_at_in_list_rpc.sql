-- Expose last message time in list RPC for UI (independent of thread.updated_at / renames).
-- Postgres does not allow changing RETURNS TABLE via CREATE OR REPLACE; drop then recreate.
DROP FUNCTION IF EXISTS list_chat_threads_for_project(uuid, uuid, boolean);

CREATE FUNCTION list_chat_threads_for_project(
  project_uuid uuid,
  user_uuid uuid,
